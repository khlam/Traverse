...
** Post-Conditions: n/a
*********************************************************************/ 
void RandomRoomba::set_initial(Floor f)
{
	max_row = f.get_row();
	max_col = f.get_col();
}
...
{	
	while(!f.isclean() && (total_moves < get_batt()) )//for (int i = 0; i < get_batt(); i++)
	{
		cout << "\n#" << total_moves << endl;
		...
		int right = -1;
		int check = 0;
		if(f.get_row_pos()-1 >= 0)
		{
			check +=1;
			up = check;
		}

		if(f.get_row_pos()+1 <= max_row)
		{
			check +=1;
			down = check;
		}

		if(f.get_col_pos()-1 >= 0)
		{
			check +=1;
			left = check;
		}

		if(f.get_col_pos()+1 <= max_col)
		{
			check +=1;
			right = check;
		}
